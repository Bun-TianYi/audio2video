...
sys.path.append("F:/My-repository/GeneFacePlusPlus-main/")
...
def extract_landmark_job(video_name, nerf=False):
    try:
        if nerf:
            ...
            out_name = video_name.replace("/videos", "/lms_2d").replace(".mp4", "_lms.npy")
        if os.path.exists(out_name):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vid_dir", default=os.path.join(root_path, 'data/raw/videos/'))
    parser.add_argument("--ds_name", default='voxceleb2')
    ...
    if ds_name.lower() == 'test':  # 处理单个视频
        ...
        # out_names = [video_name.replace("/raw", "/processed").replace("/video", "/lms_2d").replace(".mp4", "lms_3d.npy") for video_name in vid_names]
    ...
    fail_cnt = 0
    job_args = [(vid_name, ds_name == 'test') for vid_name in vid_names]
