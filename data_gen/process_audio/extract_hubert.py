...
def extract_hubert_information(audio_name):
    ### Process Single Long Audio for NeRF dataset
    current_path = os.path.dirname(os.path.abspath(__file__))
    main_path = os.path.dirname(os.path.dirname(current_path))
    # wav_16k_name = os.path.join(main_path, f"data/raw/audio/{person_id}/aud.wav")
    # hubert_npy_name = os.path.join(main_path, f"data/raw/audio/{person_id}/aud_hubert.npy")
    wav_16k_name = audio_name
    hubert_npy_name = audio_name.replace(".wav", "_hubert.npy")
    speech_16k, _ = sf.read(wav_16k_name)
...
if __name__ == '__main__':
    ...
    audio_dir = os.path.join(root_path, 'data/raw/audio/')
    audio_name_pattern = os.path.join(audio_dir, "*/*/*.wav")
    audio_names = multiprocess_glob(audio_name_pattern)
    audio_names = sorted(audio_names)
    fail_cnt=0
    for (i, res) in multiprocess_run_tqdm(extract_hubert_information, audio_names, num_workers=2,
                                          desc=f"Root {0}: extracing hubert"):
        if res is False:
            fail_cnt += 1
        print(
            f"finished {i + 1} / {len(audio_names)} = {(i + 1) / len(audio_names):.4f}, failed {fail_cnt} / {i + 1} = {fail_cnt / (i + 1):.4f}")
        sys.stdout.flush()
        pass
