...
def generate_segment_imgs_job(img_name, segmap, img,seg_model):
...
def segment_and_generate_for_image_job(img_name, img, seg, segmenter_options=None, segmenter=None, store_in_memory=False,):
    ...
    segmap_name = generate_segment_imgs_job(img_name=img_name, segmap=segmap_mask, img=img, seg_model=seg)
    if store_in_memory:
        return segmap_mask
    else:
        return segmap_name
...
def extract_segment_job(
    ...
    try:
        ...
        for i in range(len(img_lst)):
            ...
            arg = (img_name, img, seg, options, segmenter_arg, store_in_memory)
            args.append(arg)
...
if __name__ == '__main__':
    ...
    main_path = "F:/My-repository/GeneFacePlusPlus-main/"
    parser = argparse.ArgumentParser()
    parser.add_argument("--vid_dir", default=os.path.join(main_path, 'data/raw/videos/May.mp4'))
