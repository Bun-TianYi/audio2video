...
class MediapipeLandmarker:
    def __init__(self):
        ...
        model_path = os.path.join(main_path, 'data_gen/utils/mp_feature_extractors/face_landmarker.task')
    ...
    def extract_lm478_from_frames(self, frames, fps=25, anti_smooth_factor=20):
        ...
        img_lm478 = np.stack(img_mpldms)
        vid_lm478 = np.stack(vid_mpldms)
        bs, H, W, _ = frames.shape
        img_lm478 = np.array(img_lm478) * np.array([W, H, 1]).reshape([1,1,3]) # [T, 478, 2]
        vid_lm478 = np.array(vid_lm478) * np.array([W, H, 1]).reshape([1,1,3]) # [T, 478, 2]
        return img_lm478, vid_lm478
